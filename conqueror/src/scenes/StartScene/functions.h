...
std::vector<GameObject> CreateGrid(const uint8_t x_size, const uint8_t y_size, const float space, const glm::vec2 mid_pos) {
	...
	for (size_t x = 0; x < x_size; x++) {
		...
		for (size_t y = 0; y < y_size; y++) {	
			y_row.push_back(new GameObject("", Transform(glm::vec2(), glm::vec2())));	// add gameobjects to it
